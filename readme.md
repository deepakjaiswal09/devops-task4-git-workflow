...
## 📂 Project Structure
```bash
├── app.py # Sample code file
├── .gitignore # Ignored files and folders
├── README.md # Project documentation
└── docs.md # Step-by-step task report
```
...
📸 *Screenshot 1: Git repository initialization*  
...
- `feature/login` → feature branch for changes  

📸 *Screenshot 2: Branch creation and switching*  
...
📸 *Screenshot 3: .gitignore file contents*  
...
📸 *Screenshot 4: Commits pushed to GitHub*  
...
- Created a PR from `feature/login → dev`.  
...
📸 *Screenshot 6: README file on GitHub*  
...
📸 *Screenshot 7: Git tag creation and push*  
...
📸 *Screenshot 8: docs.md file preview*  
...
2. **Merge vs Rebase?** → Merge combines histories; Rebase makes history linear.  
3. **What is a Pull Request?** → A request to merge code changes from one branch to another.  
4. **How do you resolve merge conflicts?** → Manually edit conflicting files, then commit resolved changes.  
5. **What are Git tags?** → Labels pointing to specific commits (used for releases).  
6. **What is Git workflow?** → Strategy for managing branches, commits, and releases.  
7. **Explain git stash.** → Temporarily saves uncommitted changes.  
8. **What is the use of .gitignore?** → Prevents unnecessary files from being tracked.  

---
# ✅ Acknowledgement

I successfully completed DevOps Internship – Task 4, which focused on implementing a full Git workflow.
During this task I:
-Created and managed branches (main, dev, feature/login)
-Performed commits and pushes following best practices
-Raised and merged Pull Requests from feature/login → dev and dev → main
